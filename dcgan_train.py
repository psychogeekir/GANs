...
from dcgan.postProcessGenImage import restoreFullImage
...
from dcgan.stabilization_techniques import smooth_positive_labels, smooth_negative_labels, noisy_labels, \
                                           compute_gradient_penalty, genInstanceNoise

from dcgan.utils import fixSeed, generateNoise, plotDAccuracy,  \
                        plotRealBatch, plotDScore, plotLoss, plotGEvolution, getEvolutionGIF, PlotRealFake
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='folder', help='folder | cifar10 | mnist | lsun')
    ...
    parser.add_argument("--n_critic", type=int, default=1, help="interval of training generator per discriminator")
    # common values
    ...
    # --------------------------------------------------
    #    1e-4    |    0.5   |  0.900  |        5       |   WGAN_GP, LAMBDA=10
    # --------------------------------------------------
    #    1e-3    |    0.0   |  0.900  |        1       |
    # --------------------------------------------------
    ...
    parser.add_argument("--center", type=float, default=0, help="center (Lipschitz constant) to calculate of gradient penalty: 0 or 1")
    parser.add_argument("--LAMBDA", type=float, default=10, help="coefficient of gradient penalty")
    ...
    parser.add_argument("--use_instanceNoise", type=bool, default=False, help="whether to add Instance Noise to real or fake images")
    parser.add_argument("--sigma", type=float, default=1.0, help="the initial std of Gaussian Instance Noise")

    parser.add_argument("--binaryNoise", type=bool, default=False, help="whether latent variable z from bernoulli distribution, with prob=0.5")
    ...
    fixed_noise = generateNoise(opt.batch_size, opt.nz, opt.binaryNoise, Tensor)
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, _) in enumerate(dataloader):
            ...
            ###### train with real
            if opt.use_instanceNoise:
                instanceNoise = genInstanceNoise(real_imgs, sigma, device)
                out_real = discriminator(real_imgs + instanceNoise)
            else:
            ...
            z = generateNoise(imgs.shape[0], opt.nz, opt.binaryNoise, Tensor)
            ...
            if opt.use_instanceNoise:
                instanceNoise = genInstanceNoise(fake_imgs, sigma, device)
                out_fake = discriminator(fake_imgs.detach() + instanceNoise)
            else:
            ...
            d_loss = real_loss + fake_loss + gradpen
            ...
            if batches_done % opt.sample_interval == 0 or ((epoch == opt.n_epochs - 1) and (i == len(dataloader)-1)):
                ...
                gen_imgs_full = restoreFullImage(gen_imgs)
                save_image(gen_imgs_full.data[:opt.batch_size], "%s/fake_%d.png" % (opt.outTrainImgsDir, batches_done),
                ...
                gen_imgs_full = restoreFullImage(gen_imgs)
                save_image(gen_imgs_full.data[:opt.batch_size], "%s/fake_fixednoise_%d.png" % (opt.outTrainImgsDir, batches_done),
                           nrow=int(math.sqrt(opt.batch_size)), normalize=True)
                img_list.append(make_grid(gen_imgs_full, padding=2, normalize=True))
    ...
    getEvolutionGIF(batches_done, opt.sample_interval, opt.outTrainImgsDir, opt.resultPlotDir, gifName='G_Progress.gif', fps=30, loop=1)

    ## Real Images vs. Fake Images
    # Grab a batch of real images from the dataloader
    real_batch = next(iter(dataloader))
    real_batch = restoreFullImage(real_batch[0][:opt.batch_size])
    ...
    with torch.no_grad():
        ...
        z = generateNoise(nsamples, opt.nz, opt.binaryNoise, Tensor)

        # Generate a batch of images
        candidate_imgs = generator(z).detach().cpu()
        candidate_imgs_full = restoreFullImage(candidate_imgs)
        save_image(candidate_imgs_full.data[:], "%s/candidate_images_nofilter.png" % opt.outGenImgsDir, nrow=int(math.sqrt(nsamples)), normalize=True)

        # for i in range(candidate_imgs.size()[0]):
        #     candidate_imgs[i, 0] = torch.from_numpy(candidate_imgs.data[i, 0].numpy() > 0.5)
        # candidate_imgs_full = restoreFullImage(candidate_imgs)
        # save_image(candidate_imgs_full.data[:], "%s/candidate_images_threshold.png" % opt.outGenImgsDir, nrow=int(math.sqrt(nsamples)), normalize=True)

        for i in range(candidate_imgs.size()[0]):
            candidate_imgs[i, 0] = torch.from_numpy(gaussian_filter(candidate_imgs.data[i, 0].numpy(), sigma=0.8))
        candidate_imgs_full = restoreFullImage(candidate_imgs)
        save_image(candidate_imgs_full.data[:], "%s/candidate_images_Gaussian.png" % opt.outGenImgsDir, nrow=int(math.sqrt(nsamples)), normalize=True)

        for i in range(candidate_imgs.size()[0]):
            candidate_imgs[i, 0] = transforms.ToTensor()(
                transforms.ToPILImage()(candidate_imgs[i, 0]).filter(ImageFilter.SHARPEN))
        candidate_imgs_full = restoreFullImage(candidate_imgs)
        save_image(candidate_imgs_full.data[:], "%s/candidate_images_sharp.png" % opt.outGenImgsDir, nrow=int(math.sqrt(nsamples)), normalize=True)
