...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_rootpath", type=str, default='C:/Users/qlzou/OneDrive - Colostate/Research/GANs/PyTorchTutorials/data/celeba', help="root path of training images")
    ...
    parser.add_argument("--img_cropsize", type=int, default=64, help="size of each image dimension")
    ...
    parser.add_argument("--use_instanceNoise", type=bool, default=True, help="whether to add Instance Noise to real or fake images")
    ...
    dataSet = datasets.ImageFolder(root=opt.data_rootpath,
                                   transform=transforms.Compose([
                                       # transforms.CenterCrop(img_cropsize),  # crop extra white borders
                                       transforms.Resize((opt.img_size, opt.img_size)),   # resize input image to be consistent with the output of generator
                                       transforms.ToTensor(),
                                       transforms.Normalize([0.5] * opt.nc, [0.5] * opt.nc)])  # PIL image used by PyTorch is in (0, 1), bring images to (-1,1)
                                   )
    ...
    real_batch = make_grid(real_batch[0], padding=5, normalize=True).cpu()  # make a grid from a batch
